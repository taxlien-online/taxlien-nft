    ...
    return (<div className="mx-auto max-w-2xl p-4">
      ...
      <form className="mt-4 space-y-4">
        <div>
          ...
          <input className="w-full rounded border border-gray-300 p-2" id="tokenName" type="text" value={tokenName} onChange={(e) => setTokenName(e.target.value)}/>
        </div>
        <div>
          <label className="block" htmlFor="tokenSymbol">
            <b>Token symbol</b>
          </label>
          <input className="w-full rounded border border-gray-300 p-2" id="tokenSymbol" type="text" value={tokenSymbol} onChange={(e) => setTokenSymbol(e.target.value)}/>
        </div>
        <div>
          <label className="block" htmlFor="tokenSupply">
            <b>Initial token supply</b>
          </label>
          <input className="w-full rounded border border-gray-300 p-2" id="tokenSupply" type="number" value={tokenSupply} onChange={(e) => setTokenSupply(e.target.value)}/>
        </div>
        <div>
          <label className="block" htmlFor="tokenLogo">
            <b>Token Logo</b>
          </label>
          <input className="w-full rounded border border-gray-300 p-2" id="tokenLogo" type="file" accept="image/*" onChange={handleImageChange} required/>
