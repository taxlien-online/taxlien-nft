...
const BalanceChecker = ({ decimals }) => {
    ...
    const handleCheckBalance = async (e) => {
        ...
        try {
            ...
            const subaccountArray = subaccount
                ...
                : [];
            const result = await nft_taxlien_backend.icrc1_balance_of({
                owner: owner,
    ...
    return (<div className="mb-8 rounded-lg bg-white p-8 shadow-md">
      ...
      <form onSubmit={handleCheckBalance} className="space-y-6">
        {inputFields.map(({ name, value, setter, placeholder, type, required }) => (<input key={name} type={type} value={value} onChange={(e) => setter(e.target.value)} placeholder={placeholder} required={required} className="w-full rounded-md border px-3 py-2"/>))}
