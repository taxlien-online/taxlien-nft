...
import NFTTaxLienBackend "canister:nft_taxlien_backend";
...
actor {
  ...
  public shared ({ caller }) func transfer(args : TransferArgs) : async Result.Result<Icrc1Ledger.BlockIndex, Text> {
    ...
    try {
      ...
      //TODO move after OK, now strange error, don't know how to fix, don't know syntax
      //let result = await NFTTaxLienBackend.LienMint(memo);
      //debug_show(result);
